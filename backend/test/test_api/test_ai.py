...
def test_single_smart_tag_hallucinated_tag(client: TestClient):
    ...
    with patch(
        "app.services.ai_service.call_llm",
        return_value=SmartTagResult(tag_name="work", is_new=False),
    ):
...
def test_single_smart_tag_new_tag(client: TestClient):
    ...
    task_resp = client.post(
        "/tasks/", json={"title": "Test new tag", "description": "desc"}
    )
    task_id = task_resp.json()["id"]
    # No tags exist yet
    with patch(
        "app.services.ai_service.call_llm",
        return_value=SmartTagResult(tag_name="BrandNew", is_new=True),
    ):
        response = client.post(f"/ai/{task_id}")
        assert response.status_code == 200
        data = response.json()
        assert any(
            tag["tag"] == "BrandNew" for tag in data["tags"]
        )  # tag has the LLM-generated name
...
def test_single_smart_tag_existing_available_tag(client: TestClient):
    ...
    with patch(
        ...
        data = response.json()
        assert any(
            tag["tag"].lower() == "work" for tag in data["tags"]
        )  # tag has the name of the already available tag
...
def test_single_smart_tag_llm_exception(client: TestClient):
    ...
    task_id = task_resp.json()["id"]
    with patch(
        "app.services.ai_service.call_llm", side_effect=Exception("LLM crashed!")
    ):
        response = client.post(f"/ai/{task_id}")
        assert response.status_code == 503
        assert "AI service temporarily unavailable" in response.text
