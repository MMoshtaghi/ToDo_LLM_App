...
class AIService:
    ...
    def single_smart_tag(self, task_id: int, task_service:TaskService, tag_service:TagService) -> Task:
        task = task_service.get_task(task_id)
        all_tags = get_all_tags(tag_service, page_size=100) # adjust page_size as needed
        ...
        try:
            prompt = prompt_template.format(
                ...
                description=(task.description or "").replace("{", "{{").replace("}", "}}"),
